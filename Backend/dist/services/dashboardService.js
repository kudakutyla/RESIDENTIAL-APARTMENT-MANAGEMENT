...
exports.contractorDashboard = contractorDashboard;
exports.adminDashboard = adminDashboard;
const client_1 = require("../db/client");
async function tenantDashboard(userId) {
    const apartment = await (0, client_1.query)(`SELECT a.*, b.name as building_name, b.address as building_address
     FROM apartments a
     JOIN buildings b ON b.id = a.building_id
     WHERE a.tenant_id = $1
     LIMIT 1`, [userId]);
    const maintenance = await (0, client_1.query)(`SELECT status, COUNT(*)::int as count FROM maintenance_requests WHERE tenant_id = $1 GROUP BY status`, [userId]);
    const payments = await (0, client_1.query)(`SELECT status, COUNT(*)::int as count FROM payments WHERE tenant_id = $1 GROUP BY status`, [userId]);
    const docs = await (0, client_1.query)(`SELECT COUNT(*)::int as count FROM documents WHERE tenant_id = $1`, [userId]);
    return {
...
async function managerDashboard(userId) {
    const kpis = await (0, client_1.query)(`SELECT
      ...
      (SELECT COUNT(*)::int FROM security_reports sr JOIN buildings b ON b.id = sr.building_id WHERE b.manager_id = $1 AND sr.status <> 'Resolved') as open_security`, [userId]);
    return kpis.rows[0];
}
async function contractorDashboard(userId) {
    const contractor = await (0, client_1.query)(`SELECT id FROM contractors WHERE user_id = $1`, [userId]);
    const contractorId = contractor.rows[0]?.id;
    if (!contractorId) {
        return { assigned: 0, active: 0, completed: 0, highPriority: 0 };
    }
    const stats = await (0, client_1.query)(`SELECT
     ...
     WHERE assigned_contractor_id = $1`, [contractorId]);
    return stats.rows[0];
}
async function adminDashboard() {
    const summary = await (0, client_1.query)(`SELECT
    ...
    const maintenanceByStatus = await (0, client_1.query)(`SELECT status, COUNT(*)::int as count FROM maintenance_requests GROUP BY status ORDER BY status`);
    const maintenanceByCategory = await (0, client_1.query)(`SELECT category, COUNT(*)::int as count FROM maintenance_requests GROUP BY category ORDER BY category`);
    const paymentByStatus = await (0, client_1.query)(`SELECT status, COUNT(*)::int as count FROM payments GROUP BY status ORDER BY status`);
    return {
